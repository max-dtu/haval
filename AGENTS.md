...
The repo contains three independent browser modules plus a Go static file server. There is no shared bundler or framework.
...
- **JavaScript**: Keep code modular (small functions). Avoid global variables. Target elements using `data-action`, not classes or IDs (IDs are reserved for accessibility anchors). Keep it minimal.
